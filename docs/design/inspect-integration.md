...
6. ローカル検証: `scripts/run_inspect_flow.sh` を実行すると、アーティファクト生成→Inspectワーカー実行→結果表示までを一括で確認できる（デフォルトで `inspect_ai` による判定を行い、必要に応じて `INSPECT_USE_PLACEHOLDER=true` で旧来のヒューリスティック判定にフォールバック）。
